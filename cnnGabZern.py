...
class oneConv(nn.Module):
   def __init__(self, kernelSize, fLayer, inputs, imSize):
      ...
      fcIn = fcIn*fcIn*64
      self.c0 = nn.Conv2d(in_channels=inputs, out_channels=16,
                            kernel_size=kernelSize, stride=2)
      self.g0 = GaborConv2d(in_channels=inputs, out_channels=16,
                            kernel_size=kernelSize, stride=2)
      self.z0 = Zernike(in_channels=inputs, out_channels=16,
                            kernel_size=kernelSize, stride=2)
...
class threeConv(nn.Module):
   def __init__(self, kernelSize, fType, inputs):
